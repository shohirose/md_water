...
void calc_frc(vector<Atom>& atomVector, vector<int>& lj_pair_list,
              vector<int>& el_pair_list, System& system, vector<Vector3>& g);
double calc_kin(vector<Atom>& atomVector);
void calc_pot(vector<Atom>& atomVector, vector<int>& lj_pair_list,
              vector<int>& el_pair_list, System& system, vector<Vector3>& g);
...
void print_ene(int istep, System& system, double K, int nfree) {
  ...
  cout << setprecision(4) << fixed << setw(12) << istep << setw(16) << system.lj
       << setw(16) << system.es << setw(16) << totpot << setw(16) << K
       << setw(16) << totpot + K << setw(16) << K * 2. / nfree / kb << '\n';
}
...
bool load_config(ifstream& fc, System& system) {
  ...
  while (getline(fc, s)) {
    ...
    if (s.find("#", 0) != string::npos) {
      ...
      is >> stmp;
      cerr << "unknown config param \"" << stmp << "\" found.\n";
      return false;
    }
  }
